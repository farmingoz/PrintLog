...
namespace PrintLog.Hangfire.Jobs {
    public class Prisma {
        ...
        public Prisma(IServiceScopeFactory serviceScopeFactory) {
        ...
        private void MoveFile(string file, string printerName, string status) {
            if (File.Exists(file)) {
                string pathOutput = Path.Combine(Configs.RootPath, $@"Outbox\{status}", printerName);

                if (!Directory.Exists(pathOutput)) {
                    Directory.CreateDirectory(pathOutput);
                }

                string fullPath = Path.Combine(pathOutput, Path.GetFileName(file));

                int runningNo = 2;
                while (File.Exists(fullPath)) {
                    fullPath = Path.Combine(pathOutput, Path.GetFileNameWithoutExtension(file) + "_v" + runningNo + Path.GetExtension(file));
                    runningNo++;
                }
